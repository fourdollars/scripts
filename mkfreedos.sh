...
if [ -z "$1" ]; then
    echo "Usage: $0 [device node, such as /dev/sdb]"
    exit
fi
...
sudo makebootfat -o $1 -L FreeDOS -E 255 -1 fat12.bin -2 fat16.bin -3 fat32lba.bin -m /usr/lib/syslinux/mbr/mbr.bin fs-root/
cd
rm -fr $folder